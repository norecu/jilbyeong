...
const DISEASE_DESCRIPTIONS = {
  "일본뇌염":
    "일본뇌염 바이러스에 감염된 작은빨간집모기에 물려 발생하는 감염병입니다.\n" +
    "대부분 가벼운 증상을 보이지만, 일부에서는 뇌염으로 진행될 수 있습니다.\n" +
    "모기에 물리지 않도록 주의하고 예방접종을 받는 것이 중요합니다.",

  "수두":
    "수두-대상포진 바이러스에 의해 발생하며 전염성이 높은 감염병입니다.\n" +
    "발열과 함께 몸에 가려운 물집성 발진이 나타나는 것이 특징입니다.\n" +
    "환자와의 접촉을 피하고 예방접종을 통해 감염을 예방할 수 있습니다.",

  "A형간염":
    "A형간염 바이러스에 의해 발생하며 주로 오염된 음식이나 물을 통해 전파됩니다.\n" +
    "발열, 피로감, 구역질 등의 증상이 나타날 수 있으며 간 기능에 영향을 줍니다.\n" +
    "손 씻기와 안전한 음식 섭취, 예방접종을 통해 예방할 수 있습니다.",

  "쯔쯔가무시증":
    "쯔쯔가무시균을 가진 털진드기 유충에 물려 발생하는 감염병입니다.\n" +
    "발열, 두통, 근육통 등이 나타날 수 있으며 물린 부위에 가피가 생기기도 합니다.\n" +
    "야외활동 시 피부 노출을 줄이고 진드기에 물리지 않도록 주의해야 합니다.",

  "말라리아":
    "말라리아 원충에 감염된 모기에 물려 발생하는 감염병입니다.\n" +
    "발열과 오한, 두통 등의 증상이 나타날 수 있으며 적절한 치료가 필요합니다.\n" +
    "모기에 물리지 않도록 주의하고 의심 증상이 있으면 의료기관에서 진료받는 것이 중요합니다.",
};
...
function setLoading(isLoading, message) {
  if (message) {
    const paragraph = els.loading.querySelector("p");
...
const aspect = els.host.clientWidth / els.host.clientHeight;
...
function getGroundAxes() {
  const azimuthRad = THREE.MathUtils.degToRad(tiltAzimuth);
...
function updateCamera() {
  ...
  const horizontal = CAMERA_DISTANCE * Math.cos(elevationRad);

  const direction = new THREE.Vector3(
    horizontal * Math.sin(azimuthRad),
    CAMERA_DISTANCE * Math.sin(elevationRad),
...
renderer.domElement.addEventListener("pointermove", (event) => {
  ...
  target.addScaledVector(forward, -dy * unitsPerPixel);
...
function onResize() {
  const width = els.host.clientWidth;
  const height = els.host.clientHeight;
...
function updateLabels() {
  Object.entries(labelPosByCode).forEach(([code, info]) => {
    const position = screenPosition(info.pos);
...
function ringCentroidAndArea(points) {
  ...
  if (Math.abs(area) < 1e-9) {
    ...
    const x = points.reduce((sum, point) => sum + point[0], 0) / count;

    const y = points.reduce((sum, point) => sum + point[1], 0) / count;
...
function createRingLine(ring, project, height, options = {}) {
  ...
  /*
   * Border는 높이 0에서 시작한다.
   * 애니메이션에서는 position.y로 최종 높이까지 이동한다.
   */
...
function buildTerrain(featureCollection, valueByCode) {
  ...
  const maxValue = Math.max(
    1,
    ...Object.values(valueByCode).map((entry) => entry.value),
  );
  ...
  featureCollection.features.forEach((feature) => {
    ...
    const height = MIN_HEIGHT_FLOOR + Math.pow(ratio, 0.62) * maxExtrudeHeight;
    ...
    polygons.forEach((rings) => {
      ...
      /*
       * 실제 높이는 scale.y로 적용한다.
       * 따라서 Geometry의 depth는 1로 만든다.
       */
...
function startTerrainAnimation(meshes) {
  ...
  function animateTerrain(now) {
    ...
    /*
     * 모든 지역이 같은 속도로 동시에 상승한다.
     * 각 지역은 자신의 maxHeight에 도달하면 정지한다.
     */
    meshes.forEach((mesh) => {
    ...
    /*
     * Border는 scale하지 않고
     * position.y를 직접 이동시킨다.
     */
    regionGroup.traverse((object) => {
      if (!object.userData?.isTerrainBorder) {
        return;
      }

      object.position.y = progress * object.userData.finalHeight;
    });
...
function rebuildLabels(bestCentroid) {
  ...
  Object.entries(bestCentroid).forEach(([code, centroid]) => {
    ...
    els.labelsLayer.appendChild(element);
...
async function loadCaseData() {
  ...
  if (!response.ok) {
    throw new Error(
      `data.json을 불러오지 못했습니다 (HTTP ${response.status})`,
    );
  }
...
function updateSelectedSummary() {
  if (!selectedCode) {
    const { total } = buildValueByCode(selectedYear, selectedDisease);
...
async function render() {
  ...
  try {
    featureCollection = await loadSidoMap();
  } catch (error) {
    console.error(error);
  ...
  const { group, meshes, outlines, materials, maxValue, bestCentroid } =
    buildTerrain(featureCollection, lookup);
...
function setHover(code) {
  ...
  /* --------------------------------------------------------------
   * 이전 hover 제거
   * 단, selected 지역은 그대로 유지한다.
   * -------------------------------------------------------------- */

  if (hoveredCode && hoveredCode !== selectedCode) {
    if (outlinesByCode[hoveredCode]) {
      outlinesByCode[hoveredCode].forEach((outline) => {
        outline.visible = false;
      });
    }

    if (materialsByCode[hoveredCode]) {
  ...
  /* --------------------------------------------------------------
   * 새 hover 적용
   * selected 지역은 이미 selected 상태이므로
   * hover가 끝나도 selected 상태를 유지한다.
   * -------------------------------------------------------------- */
...
function handleHover(event) {
  ...
  if (hit) {
    ...
    els.tooltipValue.textContent = `${hit.object.userData.value.toLocaleString(
      "ko-KR",
    )}건`;
  } else {
    setHover(null);
